...
;-
function qtcompose, vaxis, phi

  if n_params() EQ 0 then begin
      info = 1
      USAGE:
      message, 'USAGE:', /info
      message, 'Q = QTCOMPOSE(vaxis, PHI)', info=1
      return, 0
  endif

  nph = n_elements(phi)
  nv  = n_elements(vaxis)/3
  if nph LT 1 OR nv LT 1 then goto, USAGE

  nq  = nv > nph
  q = make_array(value=vaxis[0]*phi[0]*0., 4,nq)

  sph = sin(phi/2) & cph = cos(phi/2)
  if nph EQ 1 AND nv EQ 1 then return, [ vaxis[0:2] * sph(0), cph(0) ]
  if nph GT 1 AND nv EQ 1 then begin
      ;; Single vaxis, multiple rotation angles
      q(0,*) = vaxis[0]*sph
      q(1,*) = vaxis[1]*sph
      q(2,*) = vaxis[2]*sph
      q(3,*) = cph
      ...
      q(0:2,*) = vaxis*rebin(reform(temporary(sph),1,nq),3,nq)
      q(3,*)   = temporary(cph)
  endif else begin
