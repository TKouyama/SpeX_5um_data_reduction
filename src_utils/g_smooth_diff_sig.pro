...
function g_smooth_diff_sig,data,sigma_x, sigma_y,limb=limb,cylindrical=cylindrical,ignore_value=ignore_value
  if sigma_x le 0. or sigma_y le 0. then begin
    print,"Invalid sigma"
    return, data
  endif
  ...
  ;; 配列サイズの制約 ;;
  weight_x_w = long(mergin_w*sigma_x+0.5) > 8
  weight_x_w = weight_x_w < (min([im_siz[1],im_siz[2]]) - 3.)
  weight_x = dblarr(1,weight_x_w+1)
  distance_x = (dindgen(weight_x_w+1) - weight_x_w/2)
  weight_x[0,*]= exp(-distance_x^2./(2.*sigma_x^2.))
  
  weight_y_w = long(mergin_w*sigma_y+0.5) > 8
  weight_y_w = weight_y_w < (min([im_siz[1],im_siz[2]]) - 3.)
  weight_y = dblarr(1,weight_y_w+1)
  distance_y = (dindgen(weight_y_w+1) - weight_y_w/2)
  weight_y[0,*]= exp(-distance_y^2./(2.*sigma_y^2.))
  ...
  if keyword_set(cylindrical) then begin
    ...
    tmp_data = dblarr(tmp_im_x+2*weight_x_w, tmp_im_y)
    tmp_data[0:weight_x_w-1,*] = data[tmp_im_x-1-(weight_x_w-1):tmp_im_x-1,*]
    tmp_data[weight_x_w:weight_x_w+tmp_im_x-1,*] = data
    tmp_data[tmp_im_x+weight_x_w:tmp_im_x-1+2*weight_x_w,*] = data[0:weight_x_w-1,*]
  endif else begin
    tmp_data = data
  endelse
  ...
  if keyword_set(limb) then begin
    ...
    for gj=0,im_y-1,1 do begin
      ...
      sumdata[*,gj]  = convol(tmp_g,weight_x,/EDGE_TRUNCATE)*(tmp_gV[0,*] gt 0.)
      sumcount[*,gj] = convol(tmp_gV,weight_x,/EDGE_TRUNCATE)*(tmp_gV[0,*] gt 0.)
    endfor
    ...
    for gj=0,im_y-1,1 do begin
      tmp_g[0,*] = tmp_avedata[*,gj]
      tmp_gV[0,*] = tmp_count[*,gj]
      sumdata[*,gj]  = convol(tmp_g,weight_x,/EDGE_TRUNCATE)
      sumcount[*,gj] = convol(tmp_gV,weight_x,/EDGE_TRUNCATE)
    endfor
